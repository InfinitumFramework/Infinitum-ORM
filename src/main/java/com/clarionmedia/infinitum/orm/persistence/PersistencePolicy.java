...
/**
 * <p> Provides a runtime resolution policy for model persistence based on the Infinitum configuration. There are two
 * types of persistence policies: annotation and XML. </p> <p> Domain classes should be individually registered in
 * {@code infinitum.cfg.xml} using {@code <model resource="com.foo.domain.MyModel" />} in the {@code domain} element.
 * </p>
 *
 * @author Tyler Treat
...
public abstract class PersistencePolicy {
    ...
    /**
     * Used to indicate an entity's cascade mode. {@code All} means every related entity is cascaded,
     * {@code None} means no
     * related entities are cascaded, and {@code Keys} means only foreign keys will be cascaded.
     */
     ...
     *
     * @param c the {@code Class} to check persistence for
     * @return {@code true} if persistent, {@code false} if transient
     */
    public abstract boolean isPersistent(Class<?> c);

    /**
     * Retrieves the name of the database table for the specified {@code Class}. If the {@code Class} is transient, this
     * method will return {@code null}.
     *
     * @param c the {@code Class} to retrieve the table name for
     * @return the name of the database table for the specified domain model {@code Class}
     * @throws IllegalArgumentException if the given {@code Class} is transient
     * @throws InvalidMapFileException  if the map file for the given {@code Class} is invalid
     */
    public abstract String getModelTableName(Class<?> c)
            throws IllegalArgumentException, InvalidMapFileException;

    /**
     * Retrieves a {@code List} of all persistent {@code Fields} for the given {@code Class}.
     *
     * @param c the {@code Class} to retrieve persistent {@code Fields} for
     * @return {@code List} of all persistent {@code Fields} for the specified {@code Class}
     */
     ...
     *
     * @param c the {@code Class} to retrieve the primary key {@code Field} for
     * @return the primary key {@code Field} for the specified {@code Class}
     * @throws ModelConfigurationException if multiple primary keys are declared in {@code c}
     */
    public abstract Field getPrimaryKeyField(Class<?> c)
            throws ModelConfigurationException;

    /**
     * Retrieves the name of the database column the specified {@code Field} maps to.
     *
     * @param f the {@code Field} to retrieve the column for
    ...
    /**
     * Determines if the given primary key {@link Field} is set to autoincrement. This method assumes,
     * as a precondition,
     * that the {@code Field} being passed is guaranteed to be a primary key, whether implicitly or explicitly.
     *
     * @param f the primary key {@code Field} to check if it's set to autoincrement
     * @return {@code true} if it is set to autoincrement, {@code false} if it's not
     * @throws InfinitumRuntimeException if an explicit primary key that is set to autoincrement is not of type int or
     *                                   long
     */
     ...
     *
     * @param f the {@code Field} to check if nullable
     * @return {@code true} if the field is nullable, {@code false} if it is not nullable
     */
    public abstract boolean isFieldNullable(Field f);

    /**
     * Checks if the specified {@code Field} is unique, meaning each record must have a different value in the table.
     * This
     * is a way of implementing a unique constraint on a column.
     *
     * @param f the {@code Field} to check for uniqueness
     * @return {@code true} if it is unique, {@code false} if not
     */
    public abstract boolean isFieldUnique(Field f);

    /**
     * Retrieves a {@link Set} of all {@link ManyToManyRelationship} instances for the given {@link Class}.
     *
     * @param c the {@code Class} to get relationships for
     ...
     *
     * @param c the {@code Class} to retrieve {@code Cascade} mode for
     * @return {@code Cascade} mode
     */
    public abstract Cascade getCascadeMode(Class<?> c);

    /**
     * Indicates if the given persistent {@link Field} is part of an entity relationship, either many-to-many, many
     * -to-one,
     * one-to-many, or one-to-one.
     *
     * @param f the {@code Field} to check
     * @return {@code true} if it is part of a relationship, {@code false} if not
     */
    public abstract boolean isRelationship(Field f);

    /**
     * Indicates if the given persistent {@link Field} is part of a many-to-many entity relationship.
     *
     * @param f the {@code Field} to check
     * @return {@code true} if it is part of a many-to-many relationship, {@code false} if not
     */
     ...
     *
     * @param f the {@code Field} to check
     * @return {@code true} if it is part of a one-to-one relationship, {@code false} if not
     */
    public abstract boolean isOneToOneRelationship(Field f);

    /**
     * Indicates if the given persistent {@link Field} is part of a many-to-one or one-to-one entity relationship.
     *
     * @param f the {@code Field} to check
     * @return {@code true} if it is part of a many-to-one or one-to-one relationship, {@code false} if not
     */
    public abstract boolean isToOneRelationship(Field f);

    /**
     * Retrieves the {@link ModelRelationship} the given {@link Field} is a part of.
     *
     * @param f the {@code Field} to retrieve the relationship for
     * @return the {@code ModelRelationship} for {@code f} or {@code null} if there is none
     */
    public abstract ModelRelationship getRelationship(Field f);

    /**
     * Retrieves the {@link Field} pertaining to the given {@link ModelRelationship} for the specified {@link Class}.
     * If no
     * such {@code Field} exists, {@code null} is returned.
     *
     * @param c   the {@code Class} to retrieve the {@code Field} from
     * @param rel the {@code ModelRelationship} to retrieve the {@code Field} for
    ...
    /**
     * Indicates if the given persistent {@link Class} has lazy loading enabled or not.
     *
     * @param c the {@code Class} to check lazy-loading status
     ...
     *
     * @param c the {@code Class} to retrieve the REST endpoint name for
     * @return endpoint name
     * @throws IllegalArgumentException if the given {@code Class} is not a domain model or persistent
     */
    public abstract String getRestEndpoint(Class<?> c)
            throws IllegalArgumentException;

    /**
     * Retrieves the REST endpoint field name for the given persistent {@link Field}.
     *
     * @param f the {@code Field} to retrieve the endpoint field name for
     * @return endpoint field name
     * @throws IllegalArgumentException if the containing {@link Class} of the given {@code Field} is transient or if
     * the
     *                                  {@code Field} itself is marked transient
     */
    public abstract String getEndpointFieldName(Field f)
            throws IllegalArgumentException;

    /**
     * Finds the persistent {@link Field} for the given {@link Class} which has the specified name. Returns {@code
     * null} if
     * no such {@code Field} exists.
     *
     * @param c    the {@code Class} containing the {@code Field}
     * @param name the name of the {@code Field} to retrieve
    ...
    /**
     * Retrieves a {@code List} of all unique {@code Fields} for the given {@code Class}.
     *
     * @param c the {@code Class} to retrieve unique {@code Fields} for
     * @return {@code List} of all unique {@code Fields} for the specified {@code Class}
     */
    ...
    /**
     * Calculates a hash code for the specified persistent model based on its {@link Class} and primary key.
     *
     * @param model the model entity to compute the hash for
     ...
     *
     * @param c  the {@code Class} to compute the hash for
     * @param pk the primary key to compute hash for
     * @return hash code
     */
     ...
     *
     * @param model the model to retrieve the primary key for
    ...
    /**
     * Indicates if the primary key {@link Field} for the given model is 0 or {@code null}.
     *
     * @param model the model to check the primary key value for
