...
public class SqliteBuilderTest {
    ...
    @Mock
    private OneToManyRelationship mockOtmRelationship;

    @Mock
    private ManyToManyRelationship mockMtmRelationship;
    ...
    public void setup() {
        ...
        when(mockPersistencePolicy.getPersistentFields(Long.class)).thenReturn(Arrays.asList(ArrayList.class
                .getDeclaredFields()));
        when(mockPersistencePolicy.getPersistentFields(Integer.class)).thenReturn(Arrays.asList(ArrayList.class
                .getDeclaredFields()));
    ...
    public void testCreateCountQuery_multipleCriterion_noLimitOrOffset() {
        ...
        // Run
        String expected = "SELECT count(*) FROM " + MODEL_TABLE_1 + " WHERE " + CRITERION_A_SQL + " AND " +
                CRITERION_B_SQL;
        String actual = sqliteBuilder.createCountQuery(mockCriteria);
    ...
    public void testCreateCountQuery_multipleCriterion_limitAndOffset() {
        ...
        // Run
        String expected = "SELECT count(*) FROM " + MODEL_TABLE_1 + " WHERE " + CRITERION_A_SQL + " AND " +
                CRITERION_B_SQL + " LIMIT 10 OFFSET 5";
        String actual = sqliteBuilder.createCountQuery(mockCriteria);
    ...
    public void testCreateDeleteStaleRelationshipQuery_firstType_singleRelated() {
        ...
        when(mockPersistencePolicy.getModelTableName(mockManyToManyRelationship.getFirstType())).thenReturn
                (MODEL_TABLE_1);
        when(mockPersistencePolicy.getModelTableName(mockManyToManyRelationship.getSecondType())).thenReturn
                (MODEL_TABLE_2);
        when(mockPersistencePolicy.getFieldColumnName(mockManyToManyRelationship.getFirstField())).thenReturn(COL_NAME);
        when(mockPersistencePolicy.getFieldColumnName(mockManyToManyRelationship.getSecondField())).thenReturn
                (COL_NAME);
    ...
    public void testCreateUpdateForeignKeyQuery_singleRelated() {
        ...
        // Run
        String expected = "UPDATE " + MODEL_TABLE_1 + " SET " + COL_NAME + " = " + entityPk + " WHERE " + PK_NAME + "" +
                " IN (" + id + ")";
    ...
    public void testCreateUpdateOneToOneForeignKeyQuery() {
        ...
        // Run
        String expected = "UPDATE " + MODEL_TABLE_1 + " SET " + FK_COL + " = " + relatedPk + " WHERE " + COL_NAME + "" +
                " = " + entityPk;
    ...
    public void testCreateUpdateQuery() {
        ...
        // Run
        String expected = "UPDATE " + MODEL_TABLE_1 + " SET " + COL_NAME + " = " + fk + " WHERE " + PK_NAME + " = " +
                pk;
    ...
    @Test
    public void testGetAssociationCriteriaDiscriminator_oneToMany_associatedOwner() throws NoSuchFieldException {
        // Setup
        Field pkField = Foo.class.getDeclaredField("id");
        when(mockPersistencePolicy.getPrimaryKeyField(Foo.class)).thenReturn(pkField);
        String pkCol = "id";
        when(mockPersistencePolicy.getFieldColumnName(pkField)).thenReturn(pkCol);
        when(mockPersistencePolicy.getPrimaryKeyField(Object.class)).thenReturn(pkField);
        when(mockAssociationCriteria.getRelationship()).thenReturn(mockOtmRelationship);
        when(mockOtmRelationship.getRelationType()).thenReturn(ModelRelationship.RelationType.OneToMany);
        doReturn(Foo.class).when(mockOtmRelationship).getOwner();
        String fkCol = "fk";
        when(mockOtmRelationship.getColumn()).thenReturn(fkCol);
        Field field = Foo.class.getDeclaredField("bar");
        when(mockAssociationCriteria.getRelationshipField()).thenReturn(field);
        doReturn(Foo.class).when(mockAssociationCriteria).getEntityClass();
    ...
    @Test
    public void testGetAssociationCriteriaDiscriminator_oneToMany_associatedNotOwner() throws NoSuchFieldException {
        // Setup
        Field pkField = Foo.class.getDeclaredField("id");
        when(mockPersistencePolicy.getPrimaryKeyField(Foo.class)).thenReturn(pkField);
        String pkCol = "id";
        ...
        when(mockAssociationCriteria.getRelationship()).thenReturn(mockOtmRelationship);
        when(mockOtmRelationship.getRelationType()).thenReturn(ModelRelationship.RelationType.OneToMany);
        doReturn(Object.class).when(mockOtmRelationship).getOwner();
        String fkCol = "fk";
        when(mockOtmRelationship.getColumn()).thenReturn(fkCol);
        Field relationshipField = Foo.class.getDeclaredField("bar");
        when(mockAssociationCriteria.getRelationshipField()).thenReturn(relationshipField);
    ...
    @Test
    public void testGetAssociationCriteriaDiscriminator_manyToMany_firstType() throws NoSuchFieldException {
        // Setup
        Field pkField = Foo.class.getDeclaredField("id");
        when(mockPersistencePolicy.getPrimaryKeyField(Foo.class)).thenReturn(pkField);
        String pkCol = "id";
        when(mockPersistencePolicy.getFieldColumnName(pkField)).thenReturn(pkCol);
        when(mockPersistencePolicy.getPrimaryKeyField(Object.class)).thenReturn(pkField);
        when(mockAssociationCriteria.getRelationship()).thenReturn(mockMtmRelationship);
        when(mockMtmRelationship.getRelationType()).thenReturn(ModelRelationship.RelationType.ManyToMany);
        doReturn(Object.class).when(mockMtmRelationship).getFirstType();
        Field relationshipField = Foo.class.getDeclaredField("bar");
        when(mockAssociationCriteria.getRelationshipField()).thenReturn(relationshipField);
        String relCol = relationshipField.getName();
        when(mockPersistencePolicy.getFieldColumnName(relationshipField)).thenReturn(relCol);
        doReturn(Foo.class).when(mockAssociationCriteria).getEntityClass();
        String barTable = "bar";
        when(mockPersistencePolicy.getModelTableName(Object.class)).thenReturn(barTable);
        String mtmTable = "foo_bar";
        when(mockMtmRelationship.getTableName()).thenReturn(mtmTable);
        ...
        // Run
        String expected = pkCol + " IN (SELECT " + barTable + "_" + pkCol + "_1 FROM " + mtmTable + " WHERE " +
                fooTable + "_" + pkCol + "_2 IN (SELECT " + pkCol + " FROM " + fooTable + " WHERE " + CRITERION_A_SQL
                + "))";
        String actual = sqliteBuilder.getAssociationCriteriaDiscriminator(Object.class, mockAssociationCriteria);
    ...
    @Test
    public void testGetAssociationCriteriaDiscriminator_manyToMany_secondType() throws NoSuchFieldException {
        // Setup
        Field pkField = Foo.class.getDeclaredField("id");
        when(mockPersistencePolicy.getPrimaryKeyField(Foo.class)).thenReturn(pkField);
        String pkCol = "id";
        ...
        when(mockAssociationCriteria.getRelationship()).thenReturn(mockMtmRelationship);
        when(mockMtmRelationship.getRelationType()).thenReturn(ModelRelationship.RelationType.ManyToMany);
        doReturn(Foo.class).when(mockMtmRelationship).getFirstType();
        Field relationshipField = Foo.class.getDeclaredField("bar");
        when(mockAssociationCriteria.getRelationshipField()).thenReturn(relationshipField);
        String relCol = relationshipField.getName();
        when(mockPersistencePolicy.getFieldColumnName(relationshipField)).thenReturn(relCol);
        ...
        // Run
        String expected = pkCol + " IN (SELECT " + barTable + "_" + pkCol + "_2 FROM " + mtmTable + " WHERE " +
                fooTable + "_" + pkCol + "_1 IN (SELECT " + pkCol + " FROM " + fooTable + " WHERE " + CRITERION_A_SQL
                + "))";
        String actual = sqliteBuilder.getAssociationCriteriaDiscriminator(Object.class, mockAssociationCriteria);
